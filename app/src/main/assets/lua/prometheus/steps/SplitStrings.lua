...
local function generateTableConcatNode(chunks, data)
	...
	if data.tableConcatScope and data.tableConcatId then
				...
				if math.random() < self.Threshold then
					...
					elseif self.ConcatenationType == "custom" then
						...
						elseif self.CustomFunctionType == "local" then
							local lfuncs = data.functionData and data.functionData.localFunctions;
							if not lfuncs or #lfuncs == 0 then
								-- fallback: skip local function selection
							else
