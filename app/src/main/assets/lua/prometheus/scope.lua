...
function Scope:renameVariables(settings)
	if(not self.isGlobal) then
		...
		-- FIX: Also include names from ALL higher scopes tracked in settings
		if settings._assignedNames then
			for name, _ in pairs(settings._assignedNames) do
				forbiddenNamesLookup[name] = true;
			end
		end
		...
		-- FIX: Use GLOBAL counter instead of per-scope counter to prevent
		-- name collisions between parent and child scopes.
		local globalNameId = settings._globalNameId or 0;
		...
		for id, originalName in pairs(self.variables) do
			if(not self.skipIdLookup[id] and (self.referenceCounts[id] or 0) >= 0) then
				local name;
				repeat
					name = prefix .. settings.generateName(i + globalNameId, self, originalName);
					if name == nil then
						name = originalName;
					end
					i = i + 1;
				...
				-- FIX: Track assigned name at settings level so child scopes avoid it
				if settings._assignedNames then
					settings._assignedNames[name] = true;
				end
		...
		-- FIX: Update global counter so child scopes continue from where we left off
		if settings._globalNameId then
			settings._globalNameId = settings._globalNameId + i;
		end
	end
