...
-- Cross-version readonly proxy.
-- Lua 5.1/LuaJIT have newproxy(); Lua 5.2+ removed it.
-- We use setmetatable with __newindex guard instead so the same code
-- works on every Lua version.
local function readonly(obj)
	...
	return setmetatable({}, {
		...
		__metatable = false,  -- hides metatable (getmetatable returns false)
	});
end
