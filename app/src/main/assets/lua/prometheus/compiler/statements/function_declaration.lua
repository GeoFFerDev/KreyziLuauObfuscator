...
return function(self, statement, funcDepth)
    ...
    if(#statement.indices > 0) then
        ...
        if statement.scope.isGlobal then
            ...
            if not statement.scope.isGlobal then
                local varScopeFuncDepth = self.scopeFunctionDepths[statement.scope];
    ...
    if statement.scope.isGlobal then
        ...
        local varScopeFuncDepth2 = self.scopeFunctionDepths[statement.scope];
        if varScopeFuncDepth2 == funcDepth or varScopeFuncDepth2 == nil then
            if self:isUpvalue(statement.scope, statement.id) then
                local reg = self:getVarRegister(statement.scope, statement.id, funcDepth);
