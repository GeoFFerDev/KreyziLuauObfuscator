        ...
        visitast(node, function(node, data)
            if node.kind == AstKind.Block then
                if node.scope then
                    ...
                    -- Pre-populate scopeFunctionDepths for every block scope so
                    -- that getUpvalueId never sees a nil depth.  Steps like
                    -- OpaquePredicates wrap top-level statements in a new inner
                    -- scope that was never touched by getVarRegister; without
                    -- this line those scopes stay nil and trigger the "Unresolved
                    -- Upvalue" error when the wrapped code contains assignments
                    -- that reference a variable from a parent scope.
                    if not self.scopeFunctionDepths[node.scope] then
                        self.scopeFunctionDepths[node.scope] = data.functionData.depth;
                    end
                end
            end
