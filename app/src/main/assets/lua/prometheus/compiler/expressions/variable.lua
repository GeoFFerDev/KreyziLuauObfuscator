    ...
    for i = 1, numReturns do
        if i == 1 then
            if expression.scope.isGlobal then
                ...
                -- scopeFunctionDepths[scope] may be nil for wrapper scopes
                -- created by pipeline steps (e.g. OpaquePredicates) that were
                -- never registered via getVarRegister.  Treat nil as the
                -- current funcDepth so the variable is resolved locally.
                local varScopeFuncDepth = self.scopeFunctionDepths[expression.scope];
                if varScopeFuncDepth == funcDepth or varScopeFuncDepth == nil then
