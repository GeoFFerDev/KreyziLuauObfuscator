...
-- Helper: returns true if the active lua version is 5.2 or newer (or LuaU)
local LUA52_PLUS = {
	[LuaVersion.Lua52] = true,
	[LuaVersion.Lua53] = true,
	[LuaVersion.Lua54] = true,
}
-- Helper: returns true if the active lua version is 5.3 or newer
local LUA53_PLUS = {
	[LuaVersion.Lua53] = true,
	[LuaVersion.Lua54] = true,
}
...
function Parser:statement(scope, currentLoop)
	...
	if(LUA52_PLUS[self.luaVersion] and consume(self, TokenKind.Keyword, "goto")) then
		local labelToken = expect(self, TokenKind.Ident);
		return Ast.GotoStatement(labelToken.value);
	end

	-- Label Statement ::name:: - Lua 5.2+
	if(LUA52_PLUS[self.luaVersion] and consume(self, TokenKind.Symbol, "::")) then
		local labelToken = expect(self, TokenKind.Ident);
		expect(self, TokenKind.Symbol, "::");
		return Ast.LabelStatement(labelToken.value);
	end
	...
	if(consume(self, TokenKind.Keyword, "while")) then
		...
		-- Guard: ensure body is never nil for downstream consumers
		if not stat.body then
			stat.body = Ast.Block({}, scope);
		end
		expect(self, TokenKind.Keyword, "end");
		return stat;
	end
	...
	if(consume(self, TokenKind.Keyword, "repeat")) then
		...
		-- Guard: ensure body is never nil for downstream consumers
		if not stat.body then
			stat.body = Ast.Block({}, repeatScope);
		end
...
-- Lua 5.3 bitwise precedence (low → high):
--   bor(|) > bxor(~bin) > band(&) > shift(<<,>>) > strcat(..) > add/sub > mul/div/idiv/mod > unary(not,#,-,~un) > pow(^)

function Parser:expressionComparision(scope)
	local curr = self:expressionBOr(scope);
	repeat
		local found = false;
		if(consume(self, TokenKind.Symbol, "<")) then
			local rhs = self:expressionBOr(scope);
...
-- Lua 5.3+: bitwise OR  |
function Parser:expressionBOr(scope)
	local curr = self:expressionBXor(scope);
	if LUA53_PLUS[self.luaVersion] then
		repeat
			local found = false;
			if consume(self, TokenKind.Symbol, "|") then
				local rhs = self:expressionBXor(scope);
				curr = Ast.BOrExpression(curr, rhs, true);
				found = true;
			end
		until not found;
	end
...
-- Lua 5.3+: bitwise XOR  ~ (binary)
function Parser:expressionBXor(scope)
	local curr = self:expressionBAnd(scope);
	if LUA53_PLUS[self.luaVersion] then
		repeat
			local found = false;
			-- must not consume "~=" here; tokenizer already produces "~=" as a single token so
			-- a lone "~" at this point is always binary XOR.
			if is(self, TokenKind.Symbol, "~") and not is(self, TokenKind.Symbol, "~=") then
				consume(self, TokenKind.Symbol, "~");
				local rhs = self:expressionBAnd(scope);
				curr = Ast.BXorExpression(curr, rhs, true);
				found = true;
			end
		until not found;
	end
...
-- Lua 5.3+: bitwise AND  &
function Parser:expressionBAnd(scope)
	...
	if LUA53_PLUS[self.luaVersion] then
		repeat
			...
			if consume(self, TokenKind.Symbol, "&") then
				...
				curr = Ast.BAndExpression(curr, rhs, true);
				found = true;
			end
		until not found;
	end
	...
	if LUA53_PLUS[self.luaVersion] then
		repeat
			...
			if consume(self, TokenKind.Symbol, "<<") then
				...
				curr = Ast.ShlExpression(curr, rhs, true);
				found = true;
			end
			if consume(self, TokenKind.Symbol, ">>") then
				local rhs = self:expressionStrCat(scope);
				curr = Ast.ShrExpression(curr, rhs, true);
				found = true;
			end
		until not found;
	end
...
function Parser:expressionMulDivMod(scope)
	...
	repeat
		...
		-- "//" must be tested BEFORE "/" so the longer token wins
		if LUA53_PLUS[self.luaVersion] and consume(self, TokenKind.Symbol, "//") then
			local rhs = self:expressionUnary(scope);
			curr = Ast.IdivExpression(curr, rhs, true);
			found = true;
...
function Parser:expressionUnary(scope)
	...
	-- Lua 5.3+: unary bitwise NOT  ~
	if LUA53_PLUS[self.luaVersion] and is(self, TokenKind.Symbol, "~") then
		consume(self, TokenKind.Symbol, "~");
		local rhs = self:expressionUnary(scope);
		return Ast.BNotExpression(rhs, true);
	end
