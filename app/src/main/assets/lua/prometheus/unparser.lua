...
function Unparser:unparseBlock(block, tabbing)
	...
	for i, statement in ipairs(block.statements) do
		if(statement.kind ~= AstKind.NopStatement) then
			...
			if(not self.prettyPrint) then
				-- When minifying, prepend ';' when stmt starts with '(' and follows code
				...
				-- Always join statements with ';' in minified mode
				if i ~= 1 and #code > 0 and statementCode ~= "" then
					code = code .. ";" .. statementCode
				else
					code = code .. statementCode
				end
			else
				-- Pretty-print mode unchanged
...
function Unparser:unparseStatement(statement, tabbing)
	...
	elseif(statement.kind == AstKind.GotoStatement) then
		code = "goto" .. self:whitespace() .. statement.label;

	-- Label Statement (Lua 5.2+)
...
function Unparser:unparseExpression(expression, tabbing)
	...
	local arithmeticOps = {
		...
		[AstKind.IdivExpression]  = "//",   -- Lua 5.3+
		[AstKind.ModExpression]   = "%",
		[AstKind.PowExpression]   = "^",
		[AstKind.BAndExpression]  = "&",    -- Lua 5.3+
		[AstKind.BOrExpression]   = "|",    -- Lua 5.3+
		[AstKind.BXorExpression]  = "~",    -- Lua 5.3+ binary ~
		[AstKind.ShlExpression]   = "<<",   -- Lua 5.3+
		[AstKind.ShrExpression]   = ">>",   -- Lua 5.3+
	}
	...
	k = AstKind.BNotExpression;
	if(expression.kind == k) then
		local rhs = self:unparseExpression(expression.rhs, tabbing);
		if(Ast.astKindExpressionToNumber(expression.rhs.kind) >= Ast.astKindExpressionToNumber(k)) then
			rhs = "(" .. rhs .. ")";
		end
		-- guard against ambiguous ~~ sequences
		if string.sub(rhs, 1, 1) == "~" then
			rhs = "(" .. rhs .. ")";
		end
		return "~" .. rhs;
